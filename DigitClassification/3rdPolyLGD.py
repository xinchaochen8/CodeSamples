...
def logistic_regression(X, y, learning_rate=0.01, num_iter=500):
    ...
    for i in range(num_iter):
        ...
        gradient = X.T.dot(predictions - y_adj) / N
...
def calculate_error(X, y, w):
    ...
    predictions = sigmoid(polynomial_features(X).dot(w))
...
def main():
    ...
    train_error = calculate_error(X_train_scaled, y_train, w)
    test_error = calculate_error(X_test_scaled, y_test, w)
